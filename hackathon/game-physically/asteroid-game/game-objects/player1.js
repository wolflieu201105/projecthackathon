...
class Player1 extends GameObject {
  ...
  move() {
    if (keysPressing.includes(KEY_UP1)) this.velocity.y = -PLAYER_INIT_MOVING_SPEED;
    else if (keysPressing.includes(KEY_DOWN1)) this.velocity.y = PLAYER_INIT_MOVING_SPEED;
    else this.velocity.y = 0;
    if (keysPressing.includes(KEY_LEFT1)) this.velocity.x = -PLAYER_INIT_MOVING_SPEED;
    else if (keysPressing.includes(KEY_RIGHT1)) this.velocity.x = PLAYER_INIT_MOVING_SPEED;
    else this.velocity.x = 0;
    if (this.velocity.x !== 0 && this.velocity.y !== 0)
      this.velocity = this.velocity.setLength(PLAYER_INIT_MOVING_SPEED);
    if (this.velocity.x !== 0 || this.velocity.y !== 0) {
      lastvelocity1.x = this.velocity.x;
      lastvelocity1.y = this.velocity.y;
    }
  }
  dash() {
    if (this.dashCounter.run() && keysPressing.includes(KEY_DASH1)) {
  ...
  fire() {
    if (this.fireCounter.run() && keysPressing.includes(KEY_FIRE1)) {
      const newBullet = GameObject.recycle('PlayerBullet');
      bulletPosition1.x = this.position.x + PLAYER_WIDTH / 2 - PLAYER_BULLET_SIZE / 2;
      bulletPosition1.y = this.position.y + PLAYER_HEIGHT / 2 - PLAYER_BULLET_SIZE / 2;
      newBullet.position.set(bulletPosition1);
      newBullet.velocity.set(lastvelocity1);
